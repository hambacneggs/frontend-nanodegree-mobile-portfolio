...
var resizePizzas = function(size) {
  ...
  // Rewrote changePizzaSizes function (also removed determineDx function),
  // instead of looping through and styling each div with JS, the new code
  // changes the class on the parent element #randomPizzas and that changes
  // the size of the child elements via CSS (see new styles in style.css).
  function changePizzaSizes(size) {
      var pizzaBlock = document.querySelector("#randomPizzas");
      switch (size) {
          case "1":
              pizzaBlock.classList.remove("third", "half")
              pizzaBlock.classList.add("quarter");
              break;
          case "2":
              pizzaBlock.classList.remove("quarter", "half")
              pizzaBlock.classList.add("third");
              break;
          case "3":
              pizzaBlock.classList.remove("quarter", "third")
              pizzaBlock.classList.add("half");
              break;
          default:
...
function updatePositions() {
  ...
  // moved this out of the 'for loop', it only needs to be calculated once
